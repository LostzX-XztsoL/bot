		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: 'Esse comando só pode ser usado pelo LostzX bobão🤣🤙',
					admin: 'Fica quietinho aí membro comum,tu acha que pode usar esse cmd?kkkkkkk',
					Badmin: 'Uai sô eu n sou adm como quer q eu uso esse cmdkkkkkk?',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá!\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}regist nome|idade\nExemplo : ${prefix}regist LostzX|14`,
				}
			}
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 LostzX BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 Aviso do grupo ${groupName} 」\n\n${body.slice(6)}`)
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `「 Aviso do grupo ${groupName} 」\n\n${body.slice(6)}`)
						}
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Vão se fuderkkkkkkkkk', text) // ur cods
					}, 0)
                     break
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Falhou ao converter (${tipe})`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('Só converte figurinha man pqpkkkkk')
					reply(mess.wait)
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('Esse comando só pega em vídeos amigo')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
            ...
            break
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('mamaco?')
					anu = await client.chats.all()
				...
				case 'clone':
			 	...
			 	case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Só acha por imagens amigão')
					}
					break
				default:
