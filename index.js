	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: 'Esse cmd só pode ser usado em gps amigo😐',
					ownerG: '[❗] Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: 'Só o LostzX pode usar esse comando otário🤣🤙🏻',
					admin: 'Fica quietinho aí membro comum,só os adm brabo pode usar esse cmd😎🤝🏻',
					Badmin: 'Como vc quer q eu uso esse comando se vc n me deu adm burro ;-;',
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Mlk burro pqp\né assim ó : ${prefix}regist nome|idade\nexemplo : ${prefix}regist LostzX|14`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `O registro foi bem sucedido!\n\nEm ${date} ${time}\n[Nome]: ${jeneng}\n[Número]: wa.me/${sender.split("@")[0]}\n[Idade]: ${umure}\nPara usar o bot,por favor,enviar ${prefix}help ou ${prefix}menu\n\nTotal de usuários ${user.length}`, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
